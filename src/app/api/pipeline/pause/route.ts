...
import { z } from 'zod'
import { getSessionState, pauseSession } from '@/lib/pipeline/orchestrator'
...
export async function POST(request: NextRequest): Promise<NextResponse<ApiResponse>> {
  try {
    ...
    return NextResponse.json({ success: true, data: { phase: 'paused' } })
  } catch (err) {
    console.error('POST /api/pipeline/pause:', err instanceof Error ? err.message : err)
